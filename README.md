...
- `reversa_orchestrate`: the tool behind `/reversa-auto`. The entire pipeline runs inside one tool call, which is what guarantees it never stops to ask mid-run. Each stage runs in an isolated in-process child session with no extensions, no `bash`, and `write`/`edit` restricted to `.reversa/`, the output folder, `_reversa_forward/` and — for the `docs` pipeline — `_reversa_docs/`. Git archaeology goes through a read-only `reversa_git` tool. Raw per-stage output lands in `.reversa/runs/<runId>/`.
