# Design principles — the standard the design must satisfy

Every screen, component, and flow this skill produces must uphold these. They
are distilled from clig.dev, ncurses/TUI engineering tradition, and best-in-class
TUIs (lazygit, k9s, btop, gh dash, neovim, ranger, gitui).

The numbered block below is **shared verbatim with `tui-refactor`** (which scores
an existing design against the same list). Keep the block between the markers
byte-identical across the two skills.
